...
/**
  * This provides an event sourced behavior. It has a state, [[GetvisState]], which
  ...
  */
object GetvisBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[GetvisCommand]): Behavior[GetvisCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, GetvisEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[GetvisCommand, GetvisEvent, GetvisState](
        persistenceId = persistenceId,
        emptyState = GetvisState.initial,
...
/**
  ...
  */
case class GetvisState(message: String, timestamp: String) {
  def applyCommand(cmd: GetvisCommand): ReplyEffect[GetvisEvent, GetvisState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: GetvisEvent): GetvisState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[GetvisEvent, GetvisState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[GetvisEvent, GetvisState] =
    Effect
...
object GetvisState {
  ...
  /**
    ...
    */
  def initial: GetvisState = GetvisState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[GetvisCommand]("GetvisAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[GetvisState] = Json.format
}

/**
  * This interface defines all the events that the GetvisAggregate supports.
  */
sealed trait GetvisEvent extends AggregateEvent[GetvisEvent] {
  def aggregateTag: AggregateEventTag[GetvisEvent] = GetvisEvent.Tag
}

object GetvisEvent {
  val Tag: AggregateEventTag[GetvisEvent] = AggregateEventTag[GetvisEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends GetvisEvent
...
/**
  ...
  */
trait GetvisCommandSerializable

/**
  * This interface defines all the commands that the GetvisAggregate supports.
  */
sealed trait GetvisCommand
    extends GetvisCommandSerializable
...
/**
  ...
  * application loader.
  */
object GetvisSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[GetvisState],
